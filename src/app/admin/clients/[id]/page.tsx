      ...
      <ProtectedRoute allowedRoles={['superadmin']}>
        <DashboardLayout>
          <div className="text-center py-16">
            <p className="text-gray-500 dark:text-blue-200/70 mb-4">Client not found.</p>
    ...
    <ProtectedRoute allowedRoles={['superadmin']}>
      <DashboardLayout>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="card">
            <p className="text-xs text-gray-500 dark:text-blue-200/70 uppercase tracking-wide mb-1">Total Charged</p>
            <p className="text-2xl font-bold text-gray-900 dark:text-white">{totalCharged.toLocaleString()}</p>
          </div>
          <div className="card">
            <p className="text-xs text-gray-500 dark:text-blue-200/70 uppercase tracking-wide mb-1">Total Paid</p>
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{totalPaid.toLocaleString()}</p>
          </div>
          <div className="card">
            <p className="text-xs text-gray-500 dark:text-blue-200/70 uppercase tracking-wide mb-1">Balance Due</p>
            <p className={`text-2xl font-bold ${balance > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
              {balance.toLocaleString()}
            </p>
          </div>
          <div className={`card ${isSettled ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800'}`}>
            <p className="text-xs text-gray-500 dark:text-blue-200/70 uppercase tracking-wide mb-1">Status</p>
        ...
        <div className="card">
          ...
          {transactions.length === 0 ? (
            <p className="text-gray-500 dark:text-blue-200/70 text-sm text-center py-8">
              No transactions yet.
            </p>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-navy-850">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-blue-200/70 uppercase tracking-wider">Date</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-blue-200/70 uppercase tracking-wider">Type</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-blue-200/70 uppercase tracking-wider">Description</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-blue-200/70 uppercase tracking-wider">Charged (UGX)</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-blue-200/70 uppercase tracking-wider">Paid (UGX)</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-blue-200/70 uppercase tracking-wider">Running Balance</th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-navy-950 divide-y divide-gray-100 dark:divide-gray-800">
                  {transactions.map((row) => (
                    <tr key={`${row.type}-${row.id}`} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-blue-200">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-blue-200 max-w-xs truncate">
                        {row.description}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-right font-mono text-red-600 dark:text-red-400">
                        {row.charged > 0 ? row.charged.toLocaleString() : '-'}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-sm text-right font-mono text-green-600 dark:text-green-400">
